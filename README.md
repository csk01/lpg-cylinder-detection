...
kiosk. 
A YOLOv11x detector locates the cylinder, then one or more EfficientNet classifiers
...
Identify the brand of an LPG cylinder from camera images reliably. `unknown` is a
...
- **Bottom-ring dataset growth**: 776 crops today, produced by hand; class balance is uneven, model could use more data.
- **Top-view classifier**: trained but weakest of the three (77.78%); currently used only as an conceptual,
  low-weight ensemble model. Definitely needs more data.
...
- **No held-out test set.** Due to data constraints, Only train/valid splits exist. Model selection used the validation
...
3. **Source bias.** Part of the legacy data came from Roboflow exports and YouTube frames
   (`new_yt_crops` in `build_v3_dataset.py`); frames from one video are highly correlated.
4. **Lighting / camera / background variation is undocumented.** No metadata about capture
   conditions is recorded, so robustness to a new kiosk's lighting is unknown.
5. **View distribution is uneven** — 776 bottom vs 304 top vs ~2,600 legacy side crops.
...
1. **Build a held-out test set** that is never used for model selection, and
   selection-time validation accuracy is currentenly displayed expected performance.
2. **Expand and rebalance the dataset**, especially `unknown` (93 images) and top view (304).

3. **Automate bottom-ring ROI extraction** — train a detector/segmenter for the foot ring to
   remove the manual crop bottleneck. Right now, the bottom-view model expects a cropped bottom ring image as input. 
4. **Fuse probability vectors instead of top-1 labels**, so a view's second choice still counts.
5. **Calibrate confidences** (temperature scaling) and re-derive the three thresholds from data.
6. **Dataset versioning** — manifests, checksums, per-version counts, and a changelog.
7. **Segmentation-based ROI extraction** to replace fixed percentage slices.
8. **Edge/CPU-optimised inference** — ONNX/TensorRT export; the 114 MB YOLOv11x is heavy for a
    kiosk.
9. **Production REST API** (the schema sketched in `CLAUDE.md`).
10. **Active-learning loop** feeding flagged kiosk predictions back into training.
